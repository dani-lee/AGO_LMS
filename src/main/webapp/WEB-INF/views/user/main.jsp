							...
							<table class="table table-sm">
								...
								<tbody>
									<c:if test="${empty myClassList }">
										<td colspan="5" class="text-center"><strong>학습중인 강의가 없습니다.</strong></td>
									</c:if>
									<c:forEach items="${myClassList }" var="myClass" end="4">
										<tr>
											<td class="col-md-8" style="height: 40px; cursor: pointer;" onclick="location.href='myClass/clsStudying/detail.do?opcl=${myClass.opcl}&from=mainHome'">
											<c:choose>
											...
											<c:if test="${myClass.clFormat eq 'A601'}">
												...
												</c:if> <c:if test="${myClass.clFormat eq 'A602'}">
											...
											<td class="col-md-1 text-right"><span class="badge bg-primary">${myClass.progRate}%</span></td>
							...
							<table class="table table-sm">
								...
								<tbody>
									<c:if test="${empty essentialList }">
										<td colspan="3" class="text-center"><strong>필수 강의가 없습니다.</strong></td>
									</c:if>
									<c:forEach items="${essentialList }" var="essentail" end="4">
										<tr>
											<td class="col-md-7" style="cursor: pointer;" onclick="location.href='myClass/clsStudying/detail.do?opcl=${essentail.opcl}&from=mainHome'">
												<c:choose>
											...
											<td class="col-md-2 text-right"><span class="badge bg-primary" onclick="OpenWindow('<%=request.getContextPath()%>/user/myClass/clsStudying/detail.do?opcl=${essentail.opcl}','강의상세',1100,700)" style="cursor:pointer;">강의보기</span></td>
										</tr>
									</c:forEach>
								</tbody>
							</table>
							...
							<table class="table table-sm">
								...
								<tbody>
									<c:if test="${empty dpClassInfoList }">
										<td colspan="3" class="text-center"><strong>지시 받은 강의가 없습니다.</strong></td>
									</c:if>
									<c:forEach items="${dpClassInfoList }" var="dpClassInfo" end="4">
										<tr>
											...
											<td class="col-md-2 text-right"><span class="badge bg-primary" onclick="location.href='<%=request.getContextPath()%>/user/rcls/detail.do?clCode=${dpClassInfo.clCode}&opcl=${dpClassInfo.opcl}&from=mainHome'" style="cursor:pointer;">수강신청</span></td>
										</tr>
									</c:forEach>
								</tbody>
							</table>
									...
									<c:forEach items="${finishClassList }" var="finishClass" end="4">
										<tr>
											...
											<td class="col-md-2 text-primary text-center">
												...
												<c:if test="${finishClass.intState eq 'B105' || finishClass.dDate >= date}"><span class="text-secondary">제출기한마감</span></c:if>
											</td>
						...
						<div class="card-body p-0">
							<table class="table table-sm">
								...
								<tbody>
									...
									<tr style="background-color: #F0F0F0; cursor: pointer; "  onclick="location.href='management/notice/detail.do?noticeNo=${importantNotice.noticeNo}&from=mainHome'">
										<td class="text-danger text-center col-2">[중요]</td>
										<c:choose>
											<c:when test="${fn:length(importantNotice.noticeTitle) gt 30 }">
												<td class="col-10">${fn:substring(importantNotice.noticeTitle,0,30 )}...</td>
											</c:when>
											<c:otherwise>
												<td class="col-10">${importantNotice.noticeTitle }</td>
											</c:otherwise>
										</c:choose>
									</tr>
									</c:forEach>
									<c:forEach items="${noticeList }" var="notice" end="4">
									<tr style=" cursor: pointer; "  onclick="location.href='management/notice/detail.do?noticeNo=${notice.noticeNo}&from=mainHome'">
